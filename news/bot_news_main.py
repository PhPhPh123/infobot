...
def register_statistics(bot_answer: str) -> None:
    """
    Данная функция является наблюдателем, следящим за результатами работы модулей новостей. Она передает название
    новости в объект замыкания статистики, чтобы он хранил значения всю сессию и выдавал ее после закрытия
    цикла новостей командой !stopnews бота
    @param bot_answer: строковый ответ бота
    @return: None
    """
    # Данное магическое регулярное выражение достает из полной строки квеста название самого квеста заключенное
    # в квадратные скобки, например СРОЧНЫЙ УНИКАЛЬНЫЙ КВЕСТ, название соответственно будет находится в индексе [1]
    # итогового списка, на который будет разбита строка регулярным выражением
