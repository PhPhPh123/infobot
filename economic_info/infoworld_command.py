...
from economic_info import sql_queries
...
def to_control_other_functions_and_returns_bot_answer(world_name: str, gmflag=False) -> tuple:
    ...
    # 2 этап
    tuple_with_worlds = tuple(global_main_db_cursor.execute(select_main))
    ...
    tuple_with_terrains = tuple(global_main_db_cursor.execute(select_terrains))
    tuple_with_enemies = tuple(global_main_db_cursor.execute(select_enemies))
    tuple_with_export = tuple(global_main_db_cursor.execute(select_export))
    tuple_with_import = tuple(global_main_db_cursor.execute(select_import))
...
def str_form(info_dict: dict, gmflag=False) -> tuple:
    ...
    @param gmflag: данный флаг определяет, что команда админская и вывод должен быть с игнорированием уровня доступа
    вызывать данную команду нужно в каналах, недоступных для игроков
    :return: итоговая строка-ответ для бота
    """

    if not gmflag:  # Админский флаг = False значит команда обычная, для игроков
...
Дополнительное описание и особенности: {info_dict['Дополнительное описание'] if info_dict['Уровень доступа'] > 2 else 'Неизвестно'}
    '''
    else:  # Админский флаг = True, значит команда админская, для служебного пользования
        string_answer = f'''
[АДМИНСКИЙ УРОВЕНЬ ДОСТУПА]
Наименование мира: {info_dict['Наименование мира']}
Родительская система: {info_dict['Родительская система']}
Имперский класс: {info_dict['Имперский класс']}
Имперская власть: {info_dict['Имперская власть']}
Население: {info_dict['Население']}
Относительный уровень опасности: {info_dict['Уровень опасности']}
Угрожающие враги: {info_dict['Угроза врагов']}
Основные типы местности: {info_dict['Местность']}
Экспортные товары: {info_dict['Экспортные товары']}
Импортные товары: {info_dict['Импортные товары']}
Эспортное производство: {info_dict['Экспортное перепроизводство']}
